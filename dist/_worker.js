...
`,'"',o.BYTE_ORDER_MARK],o.WORKERS_SUPPORTED=!r&&!!n.Worker,o.NODE_STREAM_INPUT=1,o.LocalChunkSize=10485760,o.RemoteChunkSize=5242880,o.DefaultDelimiter=",",o.Parser=L,o.ParserHandle=x,o.NetworkStreamer=f,o.FileStreamer=g,o.StringStreamer=y,o.ReadableStreamStreamer=b,n.jQuery&&((a=n.jQuery).fn.parse=function(u){var d=u.config||{},m=[];return this.each(function(N){if(!(a(this).prop("tagName").toUpperCase()==="INPUT"&&a(this).attr("type").toLowerCase()==="file"&&n.FileReader)||!this.files||this.files.length===0)return!0;for(var K=0;K<this.files.length;K++)m.push({file:this.files[K],inputElem:this,instanceConfig:a.extend({},d)})}),v(),this;function v(){if(m.length===0)O(u.complete)&&u.complete();else{var N,K,de,he,D=m[0];if(O(u.before)){var B=u.before(D.file,D.inputElem);if(typeof B=="object"){if(B.action==="abort")return N="AbortError",K=D.file,de=D.inputElem,he=B.reason,void(O(u.error)&&u.error({name:N},K,de,he));if(B.action==="skip")return void _();typeof B.config=="object"&&(D.instanceConfig=a.extend(D.instanceConfig,B.config))}else if(B==="skip")return void _()}var S=D.instanceConfig.complete;D.instanceConfig.complete=function(re){O(S)&&S(re,D.file,D.inputElem),_()},o.parse(D.file,D.instanceConfig)}}function _(){m.splice(0,1),v()}}),i&&(n.onmessage=function(u){u=u.data,o.WORKER_ID===void 0&&u&&(o.WORKER_ID=u.workerId),typeof u.input=="string"?n.postMessage({workerId:o.WORKER_ID,results:o.parse(u.input,u.config),finished:!0}):(n.File&&u.input instanceof File||u.input instanceof Object)&&(u=o.parse(u.input,u.config))&&n.postMessage({workerId:o.WORKER_ID,results:u,finished:!0})}),(f.prototype=Object.create(h.prototype)).constructor=f,(g.prototype=Object.create(h.prototype)).constructor=g,(y.prototype=Object.create(y.prototype)).constructor=y,(b.prototype=Object.create(h.prototype)).constructor=b,o})})(xt)),xt.exports}var ys=gs();const bs=fs(ys);function vs(e,t={}){const{maxRows:s=1e4}=t,n=bs.parse(e,{header:!0,dynamicTyping:!0,skipEmptyLines:!0,transformHeader:a=>a.trim(),preview:s});return n.errors.length>0&&console.warn("CSV parsing warnings:",n.errors.slice(0,5)),n.data}function _s(e){if(e.length===0)return{};const t=Object.keys(e[0]),s={};for(const n of t){const a=e.slice(0,Math.min(100,e.length)).map(r=>r[n]);s[n]=xs(a)}return s}function xs(e){const t=e.filter(r=>r!=null&&r!=="");if(t.length===0)return"string";if(t.filter(r=>typeof r=="number"&&!isNaN(r)).length===t.length)return"number";if(t.filter(r=>typeof r=="boolean").length===t.length)return"boolean";const a=t.filter(r=>{if(typeof r=="string"){const i=Date.parse(r);return!isNaN(i)}return!1}).length;return a===t.length&&a>0?"date":"string"}function ws(e,t){const s=[];for(const n of e){const a=n.name.toLowerCase();if(!((a.endsWith("id")||a.endsWith("_id")||a==="id")&&n.type==="number"))continue;const i=n.name.replace(/[_-]?id$/i,""),c=[`${i}Name`,`${i}_name`,`${i}name`,`${i.toLowerCase()}name`,`${i}`];for(const l of c){const o=e.find(h=>h.name.toLowerCase()===l.toLowerCase()&&h.type==="string");if(o){const h=n.unique_count/t,f=o.unique_count/t;let g=.5;Math.abs(h-f)<.2?g=.9:Math.abs(h-f)<.4&&(g=.7),s.push({id_column:n.name,name_column:o.name,confidence:g});break}}}return s.filter(n=>n.confidence>=.5)}function Es(e,t){if(t.length===0)return e;const s=new Map;for(const n of t){const a=new Map;for(const r of e){const i=r[n.id_column],c=r[n.name_column];i!=null&&c&&a.set(i,c)}s.set(n.id_column,a)}return e.map(n=>{const a={...n};for(const r of t){const i=a[r.id_column],c=s.get(r.id_column);c&&c.has(i)&&(a[`${r.id_column}_original`]=i,a[r.id_column]=c.get(i))}return a})}const vn=new qe;vn.post("/",async e=>{try{const s=(await e.req.formData()).get("file");if(!s)return e.json({error:"No file provided"},400);const n=s.name,a=n.endsWith(".csv")?"csv":n.endsWith(".json")?"json":null;if(!a)return e.json({error:"Unsupported file type. Please upload CSV or JSON."},400);if(s.size>5*1024*1024)return e.json({error:"File too large. Maximum size is 5MB."},400);const r=await s.text();let i;if(a==="csv")i=vs(r);else try{const b=JSON.parse(r);i=Array.isArray(b)?b:[b]}catch{return e.json({error:"Invalid JSON format"},400)}if(i.length===0)return e.json({error:"File contains no data"},400);if(i.length>1e4)return e.json({error:"Dataset too large. Maximum 10,000 rows supported."},400);const c=_s(i),l=Object.keys(i[0]).map(b=>({name:b,type:c[b]||"string",nullable:i.some(x=>x[b]===null||x[b]===void 0||x[b]===""),unique_count:new Set(i.map(x=>x[b])).size,sample_values:i.slice(0,3).map(x=>x[b])})),h=(await e.env.DB.prepare(`
  ...
  `).bind(e).all(),r=new Map;a.results.forEach(l=>{r.set(l.id_column,l.name_column)});let i=0;const c=[...s].sort((l,o)=>(o.quality_score||50)-(l.quality_score||50));for(const l of c){if((l.quality_score||50)<30){console.log(`Skipping low-quality visualization for ${l.column_name} (score: ${l.quality_score})`);continue}const o=await Is(l,t,r);o&&await n.prepare(`
    ...
    `).bind(t).first();if(!s)return e.json({error:"Dataset not found"},404);const n=JSON.parse(s.columns),r=(await e.env.DB.prepare(`
      SELECT * FROM analyses WHERE dataset_id = ?
    `).bind(t).all()).results.map(g=>({...g,result:JSON.parse(g.result)})),i=[],c=[],l=new Map;for(const g of n){const y=`col_${g.name}`;if(!l.has(y)){const x=10+g.unique_count/s.row_count*30;i.push({id:y,label:g.name,type:"column",size:x}),l.set(y,!0)}}const o=r.filter(g=>g.analysis_type==="correlation"),h=o.sort((g,y)=>Math.abs(y.result.correlation)-Math.abs(g.result.correlation)).slice(0,Math.min(50,o.length));for(const g of h){const{column1:y,column2:b,correlation:x}=g.result,M=`col_${y}`,L=`col_${b}`;n.length>50&&Math.abs(x)<.7||c.push({source:M,target:L,type:"correlation",strength:Math.abs(x),label:`${x>0?"+":""}${x.toFixed(2)}`})}const f=r.filter(g=>g.analysis_type==="pattern"&&(g.quality_score||0)>50);for(const g of f){const y=g.column_name;if(!y)continue;const{topPatterns:b}=g.result;if(!b||b.length===0)continue;const x=b.slice(0,3);for(const[M,L]of x){const se=`val_${y}_${M}`;l.has(se)||(i.push({id:se,label:String(M),type:"value",size:5+L/s.row_count*20}),l.set(se,!0)),c.push({source:`col_${y}`,target:se,type:"contains",strength:L/s.row_count,label:`${L}x`})}}return e.json({nodes:i,edges:c,dataset_name:s.name})}catch(t){return console.error("Relationship graph error:",t),e.json({error:"Failed to generate relationship graph"},500)}});const St=new qe;St.get("/:datasetId",async e=>{try{const t=e.req.param("datasetId"),s=await e.env.DB.prepare(`
...
Available columns: ${c.slice(0,50).map(b=>b.name).join(", ")}${c.length>50?`, ... and ${c.length-50} more`:""}
...
When users ask questions, use the appropriate tools to get actual data.`},...a,{role:"user",content:n}],h=e.env.OPENAI_MODEL||"gpt-4o-mini";console.log(`Calling OpenAI API with model: ${h} and ${Bt.length} tools`);let f="",g=[...o];for(let b=0;b<5;b++){const x=await fetch("https://api.openai.com/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${r}`},body:JSON.stringify({model:h,messages:g,tools:Bt,tool_choice:"auto",max_tokens:1500,temperature:.7})}),M=await x.text();if(!x.ok)return console.error("OpenAI API error status:",x.status),console.error("OpenAI API error response:",M),e.json({error:"Failed to get response from OpenAI",message:ot(n)},500);const L=JSON.parse(M),se=(t=L.choices)==null?void 0:t[0];if(!se)return console.error("No choice in OpenAI response:",L),e.json({error:"Empty response from OpenAI",message:ot(n)},500);const oe=se.message;if(oe.tool_calls&&oe.tool_calls.length>0){console.log(`Tool calls requested: ${oe.tool_calls.length}`),g.push(oe);for(const ee of oe.tool_calls){const ue=ee.function.name,J=JSON.parse(ee.function.arguments);console.log(`Executing tool: ${ue}`,J);let O;try{switch(ue){case"get_outlier_columns":O=await zs(e.env.DB,s,J);break;case"get_correlation_analysis":O=await Bs(e.env.DB,s,J);break;case"get_column_statistics":O=await Us(e.env.DB,s,J);break;case"search_analyses":O=await Ws(e.env.DB,s,J);break;case"get_data_sample":O=await Js(e.env.DB,s,J);break;case"get_missing_values":O=await Vs(e.env.DB,s,J);break;case"suggest_data_cleaning":O=await Ks(e.env.DB,s,J);break;default:O={error:`Unknown function: ${ue}`}}}catch(u){console.error(`Tool execution error for ${ue}:`,u),O={error:`Failed to execute ${ue}: ${u}`}}g.push({role:"tool",tool_call_id:ee.id,content:JSON.stringify(O)})}continue}f=oe.content||"";break}if(!f)return e.json({error:"No final response from OpenAI",message:ot(n)},500);const y=Gs(n,[]);return e.json({message:f,suggestions:y})}catch(s){console.error("Chat error:",s);const n=s instanceof Error?s.message:String(s);return e.json({error:"Chat failed: "+n,message:ot("error")},500)}});function Gs(e,t){const s=e.toLowerCase(),n=[];return s.includes("outlier")?(n.push("How should I clean the outliers?"),n.push("Show me correlations between outlier columns")):s.includes("correlat")?(n.push("Which columns have the most outliers?"),n.push("Show me a data sample")):s.includes("clean")?(n.push("Show me missing values"),n.push("What are the biggest data quality issues?")):(n.push("Which columns have outliers?"),n.push("Suggest data cleaning steps"),n.push("Show me the strongest correlations")),n.slice(0,3)}function ot(e){const t=e.toLowerCase();return t.includes("outlier")?"To see outliers, go to the 'Insights' tab and search for 'outlier'.":t.includes("correlat")?"Check the 'Insights' tab and search for 'correlation'.":"I'm currently operating in fallback mode. Please configure your OpenAI API key."}const ve=new qe;ve.use("/api/*",ss());ve.use("/static/*",ps({root:"./public"}));ve.route("/api/upload",vn);ve.route("/api/datasets",nt);ve.route("/api/analyze",_n);ve.route("/api/relationships",xn);ve.route("/api/mappings",St);ve.route("/api/chat",wn);ve.get("/api/health",e=>e.json({status:"ok",timestamp:new Date().toISOString()}));ve.get("/",e=>e.html(`
    <!DOCTYPE html>
    <html lang="en">
    <head>
        <meta charset="UTF-8">

...
function parseNumericString(raw: string): NumericParseResult {
  let text = raw.trim();
  if (!text) {
...
function detectDate(value: string): { isDate: boolean; isDateTime: boolean } {
...
function computeSkewness(values: number[]): number | undefined {
  if (values.length < 3) {
    return undefined;
  }
  const mean =
  ...
  for (const columnName of columnNames) {
    ...
    for (const rawValue of sampleValues) {
      if (typeof rawValue === 'number' && Number.isFinite(rawValue)) {
        ...
        if (Number.isInteger(rawValue)) integerCount += 1;
        if (rawValue > 0) positiveCount += 1;
        if (rawValue === 0 || rawValue === 1) zeroOneCount += 1;
        continue;
      }
      ...
      if (numericParse.isNumeric && numericParse.value !== undefined) {
        ...
        if (Number.isInteger(numericParse.value)) integerCount += 1;
        if (numericParse.value > 0) positiveCount += 1;
        if (numericParse.value === 0 || numericParse.value === 1) zeroOneCount += 1;
        if (numericParse.isCurrency) currencyCount += 1;
        if (numericParse.isPercentage) percentageCount += 1;
        continue;
      }
    ...
    const numericStats =
      baseType === 'number' && numericValues.length > 0
        ? {
    ...
    if (numericStats && numericStats.skewness && Math.abs(numericStats.skewness) > 1.5) {
      notes.push(
        `Highly skewed distribution detected (skewness ${numericStats.skewness.toFixed(2)}).`
      );
    }

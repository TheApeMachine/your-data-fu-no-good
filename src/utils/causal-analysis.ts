...
export interface GrangerCausalityResult {
  ...
  significant: boolean;
  confidence: number;
  direction: 'unidirectional' | 'bidirectional' | 'no_causality';
}
...
export interface CausalityInsight {
  type: 'granger' | 'conditional_independence' | 'correlation_causality';
  summary: string;
  confidence: number;
  evidence: string[];
  caveats: string[];
...
function linearRegression(
  y: number[],
  X: number[][]
): { residuals: number[]; rSquared: number } {
  const n = y.length;
  const k = X[0].length;
...
export function grangerCausalityTest(
  ...
  const fStatistic =
    ((rssRestricted - rssUnrestricted) / (kUnrestricted - kRestricted)) /
 ...
 */
export function generateCausalityInsights(
  correlations: Array<{
    column1: string;
    column2: string;
    correlation: number;
  ...
  const insights: CausalityInsight[] = [];
 ...
 */
export function assessCausalAnalysisFeasibility(

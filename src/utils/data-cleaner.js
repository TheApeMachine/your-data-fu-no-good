 ...
 */
function removeRowsWithTooManyEmpties(rows, columns, threshold) {
 ...
 */
function trimWhitespace(rows, columns) {
 ...
 */
function removeRowsWithEmptyKeys(rows, keyColumns) {
    return rows.filter(row => {
 ...
 */
function standardizeText(rows, columns) {
    const stringColumns = columns.filter(c => c.type === 'string').map(c => c.name);
    return rows.map(row => {
        const cleaned = { ...row };
        for (const col of stringColumns) {
 ...
 */
function fillNumericGaps(rows, columns) {
 ...
 */
export function suggestKeyColumns(rows, columns) {
    const suggestions = [];

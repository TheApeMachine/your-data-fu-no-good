...
function friendlyTransformationName(transformation) {
    return FEATURE_LABELS[transformation] ?? transformation.replace(/_/g, ' ');
}
function buildFeatureActionLabel(transformation, columns) {
...
function composeNarrative(narrative) {
    const segments = [];
...
function describeUniqueness(stats) {
...
export function describeStatistics(column, stats) {
    const count = stats.count || 0;
    ...
    if (column.type === 'number') {
        const typical = toNumber(stats.median) ??
            toNumber(stats.mean) ??
            toNumber(stats.mode);
        ...
        const coeffVar = stdDev !== undefined && mean !== undefined && mean !== 0
...
export function describeMissingData(column, result, stats) {
...
export function describeOutliers(column, result, stats) {
...
export function describeTrend(column, trend, stats) {
    const percentStrength = Math.round(trend.strength * 100);
    const directionLabel = trend.direction === 'up' ? 'rising' : trend.direction === 'down' ? 'declining' : 'stable';
    const narrative = {
...
export function describeAnomalies(column, result, stats) {
...
export function describeCorrelation(column1, column2, metrics) {
    let pearson = null;
    let spearman = null;
    let normalizedMI = null;
    let sampleSize = null;
    let bestMetric = null;
    ...
    if (normalizedMI !== null && normalizedMI > 0)
    ...
    const signalParts = [];
    if (pearson !== null)
        signalParts.push(`Pearson: ${formatSigned(pearson)}`);
    if (spearman !== null)
        signalParts.push(`Spearman: ${formatSigned(spearman)}`);
    if (normalizedMI !== null)
        signalParts.push(`NMI: ${normalizedMI.toFixed(2)}`);
    if (sampleSize !== null)
        signalParts.push(`Rows: ${Math.round(sampleSize).toLocaleString()}`);
...
export function describeTimeSeriesInsight(dateColumn, valueColumn, stats) {
...
export function describePattern(column, result) {
    ...
    narrative.signal = `Top categories cover ${formatPercent(Math.min(result.topPatterns.slice(0, 3).reduce((acc, pattern) => acc + pattern.percentage, 0) / 100, 1))} of rows`;
    const actions = [];
...
function summarizeClusterProfile(cluster) {
    const highs = [];
    const lows = [];
...
export function describeClustering(result) {
    ...
    const actions = result.clusters.map((cluster, index) => ({
        id: 'view_cluster',
...
export function describeJoinSuggestion(leftDataset, rightDataset, suggestion) {
...
export function describeFeatureSuggestion(suggestion) {
...
export function describePCA(result) {

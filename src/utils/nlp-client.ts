...
export async function analyzeSentiment(texts: string[]): Promise<SentimentResult[]> {
  ...
  try {
    ...
    return texts.map(() => ({ label: 'neutral' as const, confidence: 0.5 }));
  }
}
...
export async function classifyIntent(
  ...
  try {
    ...
    return texts.map(() => ({ label: 'other' as const, confidence: 0.5 }));
  }
}
 ...
 */
export async function getAggregatedIntent(
  texts: string[]
): Promise<{ label: 'question' | 'complaint' | 'request' | 'information' | 'feedback' | 'other'; confidence: number }> {
  if (texts.length === 0) {

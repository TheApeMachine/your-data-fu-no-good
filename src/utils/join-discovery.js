...
function computeStatConfidence(ratioA, ratioB) {
                ...
                for (const leftCol of left.columns) {
                    for (const rightCol of right.columns) {
                        ...
                        if (leftCol.type === 'string' && leftCol.profile?.minhash && rightCol.profile?.minhash) {
                            ...
                            valueSimilarity = computeStatConfidence(getUniqueRatio(leftCol, left.row_count), getUniqueRatio(rightCol, right.row_count));
                        }
                        if (valueSimilarity < 0.2)

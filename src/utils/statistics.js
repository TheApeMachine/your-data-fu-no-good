...
export function calculateStats(values, columnType) {
...
export function detectOutliers(numbers) {
    if (numbers.length < 4)
...
export function detectRobustAnomalies(series, options = {}) {
...
function discretizeIntoBins(values, desiredBins) {
...
export function calculateMutualInformationMetrics(x, y, maxBins = 10) {
    if (x.length !== y.length || x.length === 0) {
    ...
    const xCounts = new Array(xBinCount).fill(0);
    const yCounts = new Array(yBinCount).fill(0);
    const jointCounts = Array.from({ length: xBinCount }, () => new Array(yBinCount).fill(0));
 ...
 */
export function analyzeTimeSeries(dates, values) {
    if (dates.length !== values.length || dates.length < 2)
        return null;
    // Parse dates and create time-series points
    const points = [];
    for (let i = 0; i < dates.length; i++) {
        const date = dates[i] instanceof Date ? dates[i] : parseDate(dates[i]);
    ...
    let granularity;
    if (avgGap < 3600000 * 6)
        granularity = 'hourly'; // < 6 hours
    else if (avgGap < 86400000 * 3)
        granularity = 'daily'; // < 3 days
    else if (avgGap < 86400000 * 10)
        granularity = 'weekly'; // < 10 days
    else if (avgGap < 86400000 * 45)
        granularity = 'monthly'; // < 45 days
    else
        granularity = 'yearly';
    // Calculate trend
 ...
 */
function detectSeasonality(points, granularity) {
    if (points.length < 14)
 ...
 */
function checkPeriodicCorrelation(points, period) {
 ...
 */
export function aggregateTimeSeries(points, aggregation) {
    if (points.length === 0)
        return [];

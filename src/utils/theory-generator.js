 ...
 */
export function generateMissingDataTheories(profile) {
    const theories = [];
        ...
        theories.push({
            ...
            suggestedActions: [
                ...
                },
                {
                    ...
                    priority: highMissing.length > 2 ? 'high' : 'medium'
                }
            ],
 ...
 */
export function generateCorrelationTheories(profile) {
    const theories = [];
    const correlationAnalyses = profile.analyses.filter(a => a.type === 'correlation');
    if (correlationAnalyses.length === 0)
        return theories;
 ...
 */
export function generateAnomalyTheories(profile) {
    const theories = [];
    const anomalyAnalyses = profile.analyses.filter(a => a.type === 'anomaly' || a.type === 'outlier');
 ...
 */
export function generateTrendTheories(profile) {
    const theories = [];
 ...
 */
export async function generateAllTheories(db, datasetId) {
 ...
 */
async function fetchDatasetProfile(db, datasetId) {
    ...
    // Fetch analyses
    const analysesResult = await db.prepare('SELECT analysis_type, column_name, result, confidence FROM analyses WHERE dataset_id = ?').bind(datasetId).all();

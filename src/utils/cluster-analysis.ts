...
export interface Cluster {
  ...
  size: number;
  profile: Record<string, { mean: number; min: number; max: number; description: 'low' | 'average' | 'high' }>;
  label?: string;
}
...
export interface ClusteringResult {
  columns: string[];
  clusters: Cluster[];
  totalWithinClusterSumOfSquares: number;
  elbowScore?: number;
}
...
function normalize(data: number[][]): number[][] {
  ...
  for (let j = 0; j < numFeatures; j++) {
    ...
    const range = max - min;
    if (range < 1e-9) {
      for (let i = 0; i < numSamples; i++) normalized[i][j] = 0;
    } else {
      for (let i = 0; i < numSamples; i++) {
...
function runKMeans(data: number[][], k: number): { clusters: number[][]; assignments: number[] } {
  const numSamples = data.length;
  const numFeatures = data[0].length;
  ...
  for (let iter = 0; iter < MAX_ITERATIONS && changed; iter++) {
    ...
    // Update step
    const newCentroids: number[][] = Array.from({ length: k }, () => new Array(numFeatures).fill(0));
...
function calculateWCSS(data: number[][], assignments: number[], centroids: number[][]): number {
  let wcss = 0;
...
function getColumnProfile(
  clusterCentroidValue: number,
  overallStats: { mean: number; stdDev: number, min: number, max: number }
): { mean: number; min: number; max: number; description: 'low' | 'average' | 'high' } {
  const { mean, stdDev } = overallStats;
  const oneStdDev = stdDev || 0.001;

  let description: 'low' | 'average' | 'high' = 'average';
...
export function findOptimalClusters(
  ...
  const overallColumnStats: Record<string, { mean: number, stdDev: number, min: number, max: number }> = {};

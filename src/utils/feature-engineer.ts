...
function createSuggestionKey(suggestion: FeatureSuggestion): string {
  return `${suggestion.transformation}:${suggestion.columns.join('|')}`;
}

export function deriveFeatureSuggestions(
  ...
  const addSuggestion = (suggestion: FeatureSuggestion) => {
    ...
    if (profile.null_count > 0) {
      ...
      if (nullPercentage >= 5) {
        addSuggestion({
          ...
          description: `${columnName} is empty in ${profile.null_count} rows (${nullPercentage.toFixed(
            1
          )}%). Adding a simple yes/no "was missing" column keeps that information visible in charts and filters.`,
          columns: [columnName],

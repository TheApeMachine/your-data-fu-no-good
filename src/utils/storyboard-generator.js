 ...
 */
function groupInsightsByTheme(analyses) {
 ...
 */
function generateExecutiveSummary(analyses, datasetName, rowCount) {
    const insights = analyses.length;
    const highImportance = analyses.filter(a => a.importance === 'high' || a.importance === 'critical').length;
    const criticalIssues = analyses.filter(a => a.analysis_type === 'anomaly' ||
 ...
 */
function generateSectionNarrative(theme, analyses) {
    const sortedByImportance = [...analyses].sort((a, b) => {
        const importanceOrder = { 'critical': 0, 'high': 1, 'medium': 2, 'low': 3 };
        return (importanceOrder[a.importance] || 3) -
 ...
 */
function generateRecommendedActions(analyses) {
    const actions = [];
    ...
    const strongCorrelations = analyses.filter(a => a.analysis_type === 'correlation' && Math.abs(a.result?.best_strength || 0) > 0.8);
    ...
    const strongTrends = analyses.filter(a => (a.analysis_type === 'trend' || a.analysis_type === 'timeseries') &&
 ...
 */
function generateKeyTakeaways(analyses, actions) {
    const takeaways = [];
    // Data quality takeaway
    const qualityIssues = analyses.filter(a => a.analysis_type === 'missing' || a.analysis_type === 'anomaly' || a.analysis_type === 'outlier');
    ...
    const strongCorr = analyses.filter(a => a.analysis_type === 'correlation' && Math.abs(a.result?.best_strength || 0) > 0.7);
    if (strongCorr.length > 0) {
        takeaways.push(`${strongCorr.length} strong relationships discovered between variables`);
    }
    // Temporal takeaway
    const trends = analyses.filter(a => (a.analysis_type === 'trend' || a.analysis_type === 'timeseries') && (a.result?.strength || 0) > 0.5);
    if (trends.length > 0) {
 ...
 */
export async function generateStoryboard(db, datasetId) {
    // Fetch dataset info
    ...
    if (!dataset)
        return null;
    // Fetch all analyses
    const analysesResult = await db.prepare('SELECT * FROM analyses WHERE dataset_id = ? ORDER BY quality_score DESC, importance DESC, confidence DESC').bind(datasetId).all();
    ...
    const vizResult = await db.prepare('SELECT id, chart_type, title, analysis_id FROM visualizations WHERE dataset_id = ?').bind(datasetId).all();
    ...
    for (const [theme, themeAnalyses] of themes.entries()) {
        ...
        const importance = hasHighImportance && avgConfidence > 0.7 ? 'critical' :
    ...
    const vizReferences = visualizations.map(v => {
 ...
 */
export function exportStoryboardAsMarkdown(storyboard) {

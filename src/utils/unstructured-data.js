 ...
 */
export function isLogFormat(text) {
    // Common log patterns
    const logPatterns = [
...
export function parseLogEntry(text) {
    const entry = { message: text, raw: text };
    ...
    if (levelMatch) {
        entry.level = (levelMatch[1] || levelMatch[2]).toUpperCase();
 ...
 */
export function extractPatterns(texts) {
    const patterns = [];
    // IP addresses
 ...
 */
export function analyzeUnstructuredData(texts, sampleSize = 100) {
    ...
    if (isLogData && logEntries && logEntries.length > 0) {
        ...
        structuredFeatures.avg_metadata_fields = logEntries.reduce((sum, e) => sum + Object.keys(e.metadata || {}).length, 0) / logEntries.length;
    }
    return {
 ...
 */
export function generateUnstructuredInsights(analysis) {
    const insights = [];

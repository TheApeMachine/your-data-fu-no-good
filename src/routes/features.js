...
function sanitizeValueForColumnName(value) {
...
function makeUniqueColumnName(baseName, existingNames) {
...
async function fetchDatasetMeta(db, datasetId) {
...
async function fetchDatasetRows(db, datasetId) {
...
async function persistRows(db, rows) {
    const statements = [];
...
async function saveColumnDefinitions(db, datasetId, columns) {
    await db
        .prepare(`UPDATE datasets SET columns = ?, column_count = ? WHERE id = ?`)
        .bind(JSON.stringify(columns), columns.length, datasetId)
        .run();
}
function getAccumulator(map, columnName) {
    let accumulator = map.get(columnName);
    if (!accumulator) {
        accumulator = { samples: [], uniqueValues: new Set(), nullCount: 0 };
        map.set(columnName, accumulator);
    }
    return accumulator;
}
function recordColumnValue(map, columnName, value) {
...
function buildColumnDefinition(name, type, totalRows, accumulator, options = {}) {
...
function requireColumn(columns, columnName) {
...
async function applyMissingIndicator(datasetId, column, db) {
...
async function applyLogHelper(datasetId, column, db) {
    const { columns } = await fetchDatasetMeta(db, datasetId);
    requireColumn(columns, column);
    const existingNames = new Set(columns.map((col) => col.name));
...
async function applyStandardizeHelper(datasetId, column, db) {
    const { columns } = await fetchDatasetMeta(db, datasetId);
    requireColumn(columns, column);
    const existingNames = new Set(columns.map((col) => col.name));
    ...
    for (const row of rows) {
        ...
        row.data[newColumnName] = Number.isFinite(zScore) ? zScore : null;
...
async function applyOneHotEncoding(datasetId, column, db) {
    const { columns } = await fetchDatasetMeta(db, datasetId);
    requireColumn(columns, column);
    const existingNames = new Set(columns.map((col) => col.name));
    const rows = await fetchDatasetRows(db, datasetId);
    ...
    await persistRows(db, rows);
    const newColumns = categoryColumns.map((category) => {
...
function describeHelperColumns(columnNames) {
    if (columnNames.length === 1)
        return `helper column "${columnNames[0]}"`;
    if (columnNames.length === 2)
        return `helper columns "${columnNames[0]}" and "${columnNames[1]}"`;
    if (columnNames.length === 3)
        return `helper columns "${columnNames[0]}", "${columnNames[1]}" and "${columnNames[2]}"`;
    return `${columnNames.length} helper columns`;
}
async function applyDatetimeComponents(datasetId, column, db) {
    const { columns } = await fetchDatasetMeta(db, datasetId);
    requireColumn(columns, column);
    const existingNames = new Set(columns.map((col) => col.name));
    const rows = await fetchDatasetRows(db, datasetId);
    const totalRows = rows.length;
    const componentDefs = [
        { key: 'year', getter: (date) => date.getFullYear(), semantic: 'date_year' },
        { key: 'quarter', getter: (date) => Math.floor(date.getMonth() / 3) + 1, semantic: 'date_quarter' },
        { key: 'month', getter: (date) => date.getMonth() + 1, semantic: 'date_month' },
        { key: 'day', getter: (date) => date.getDate(), semantic: 'date_day' },
        { key: 'weekday', getter: (date) => date.getDay(), semantic: 'date_weekday' },
    ];
    ...
    await persistRows(db, rows);
    const newColumns = columnMappings.map((mapping) => buildColumnDefinition(mapping.columnName, 'number', totalRows, statsMap.get(mapping.columnName), {
...
async function applyExtractDomain(datasetId, column, db) {
    const { columns } = await fetchDatasetMeta(db, datasetId);
    requireColumn(columns, column);
    const existingNames = new Set(columns.map((col) => col.name));
    ...
    let extracted = 0;
    for (const row of rows) {
        const value = typeof row.data[column] === 'string' ? row.data[column] : null;
...
async function applyExtractHost(datasetId, column, db) {
    const { columns } = await fetchDatasetMeta(db, datasetId);
    requireColumn(columns, column);
    const existingNames = new Set(columns.map((col) => col.name));
...
async function applyIdentifierEncoding(datasetId, column, db) {
    const { columns } = await fetchDatasetMeta(db, datasetId);
    requireColumn(columns, column);
    const existingNames = new Set(columns.map((col) => col.name));

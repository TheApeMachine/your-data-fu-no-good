...
function extractColumnValue(data, column) {
...
datasets.get('/:id/segments', async (c) => {
    try {
        ...
        const targetRows = typeof rowsQuery === 'string' && rowsQuery.length > 0
            ? rowsQuery
                .split(',')
        ...
        if (typeof equalsValue === 'string' && equalsValue.length > 0) {
            ...
            candidates = candidates.filter((entry) => entry.numericValue !== null && Number.isFinite(entry.numericValue));

...
mongodbUpload.post('/', async (c) => {
    try {
        const { connectionString, database, collection, query, pipeline, limit } = await c.req.json();
        // Validate inputs
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).bind(datasetName, `${database}.${collection}`, 'mongodb', rows.length, columns.length, JSON.stringify(columns), 'analyzing').run();
